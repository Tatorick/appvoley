    ...
    const fetchSessions = React.useCallback(async () => {
        ...
        try {
            ...
            const { data, error } = await supabase
                ...
                .select(`
                *,
                teams (nombre, categories(nombre)),
                attendance (status)
            `)
                .eq('club_id', club.id)
                        ...
                        return (
                            <div key={session.id} className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-all flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div className="flex items-start gap-4">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-slate-800 text-lg">{session.teams?.nombre} <span className="text-xs text-slate-400 font-normal">({session.teams?.categories?.nombre})</span></h3>
