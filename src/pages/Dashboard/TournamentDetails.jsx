...
export default function TournamentDetails() {
    ...
    const fetchTournamentDetails = async () => {
        ...
        try {
            ...
            // 2. Fetch Roster (without nested teams to avoid 400 error)
            const { data: rData, error: rError } = await supabase
                .from('tournament_roster')
                .select('*, players(id, first_name, last_name, position, team_id)')
                .eq('tournament_id', id)

            if (rError) throw rError

            // 2b. Fetch Team Names manually
            const teamIds = [...new Set(rData?.map(r => r.players?.team_id).filter(Boolean))]
            let teamsMap = {}
            if (teamIds.length > 0) {
            ...
            const rosterWithTeams = rData?.map(r => ({
                ...
                players: {
                    ...r.players,
                    teams: { nombre: teamsMap[r.players?.team_id] || 'Sin Equipo' }
                }
            })) || []
            ...
            // Fetch all teams for the club to map names
            ...
            const allTeamsMap = {}
            allTeams?.forEach(t => allTeamsMap[t.id] = t.nombre)

            const allPlayersWithTeams = apData?.map(p => ({
                ...p,
                teams: { nombre: allTeamsMap[p.team_id] || 'Sin Equipo' }
            })) || []
